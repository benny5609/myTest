...
#define SKINFILE_NAME "PSClient.xml"
...
public:
	...
	{
		ShellExecute(0, NULL, _T( "http://www.acejoy.com"), NULL, NULL, SW_NORMAL);
	}
	...
	void OnMessage_TestServer(const char* pServerIP, int nPort, int nThreadCount, int nSendCount)
	{
		...
		if(true == m_ClientConnect.Init(pServerIP, nPort, nThreadCount, nSendCount, nTimeCost))
		{
	...
	void OnMessage_ShowClientCount()
	{
		...
		sprintf_s(szText, 300, _T("当前CPU占用率(%f%%), 当前内存占用为%fMb。"), (float)ServerRunInfo.m_nCPU/100, (float)ServerRunInfo.m_nMemorySize/(1000*1000));
	...
	void OnMessage_ShowForbiddenIP(const char* pCommand)
	{
		...
		for(int i = 0; i < (int)m_VecForbiddenIP.size(); i++)
		{
			...
			if(NULL != pListElement)
			{
				...
				if(m_VecForbiddenIP[i].m_nType == 0)
				{
	...
	void OnMessage_ShowServerConnectInfoTCP(const char* pCommand)
	{
		...
		for(int i = 0; i < (int)m_VecServerTCPConnectInfo.size(); i++)
		{
			CListTextElementUI* pListElement = new CListTextElementUI;
			if(NULL != pListElement)
			{
	...
	void OnMessage_ShowServerConnectInfoUDP(const char* pCommand)
	{
		...
		for(int i = 0; i < (int)m_VecServerUDPConnectInfo.size(); i++)
		{
			CListTextElementUI* pListElement = new CListTextElementUI;
			if(NULL != pListElement)
			{
	...
	void OnMessage_ShowWorkUDPClientInfo(const char* pCommand)
	{
		...
		for(int i = 0; i < (int)m_VecClientConnectInfo.size(); i++)
		{
			CListTextElementUI* pListElement = new CListTextElementUI;
			if(NULL != pListElement)
			{
	...
	void OnMessage_ShowAllCommand(const char* pCommand)
	{
		...
		pList->RemoveAll();
		//pList->SetTextCallback(this);
		for(int i = 0; i < (int)m_VecCommandInfo.size(); i++)
		{
			CListTextElementUI* pListElement = new CListTextElementUI;
			if(NULL != pListElement)
			{
	...
	void OnMessage_ShowConnectHistory(const char* pCommand)
	{
		...
		for(int i = 0; i < (int)m_VecIPAccount.size(); i++)
		{
			CListTextElementUI* pListElement = new CListTextElementUI;
			if(NULL != pListElement)
			{
	...
	void OnMessage_ShowWorkThreadInfo(const char* pCommand)
	{
		...
		pList->RemoveAll();
		//pList->SetTextCallback(this);
		for(int i = 0; i < (int)m_VecWorkThreadInfo.size(); i++)
		{
			CListTextElementUI* pListElement = new CListTextElementUI;
			if(NULL != pListElement)
			{
	...
	void OnMessage_ShowModule()
	{
		...
		pList->RemoveAll();
		//pList->SetTextCallback(this);
		for(int i = 0; i < (int)m_VecModuleInfo.size(); i++)
		{
			CListTextElementUI* pListElement = new CListTextElementUI;
			if(NULL != pListElement)
			{
	...
	LPCTSTR GetItemText(CControlUI* pControl, int iIndex, int iSubItem)
	{
		...
		if(pControl->GetName() == _T("domainlist"))
		{
			...
			case 0:
				{
					_stprintf(szBuf, m_VecModuleInfo[iIndex].szModuleFile);
					break;
				}
			case 1:
				{
					_stprintf(szBuf, m_VecModuleInfo[iIndex].szModuleName);
					break;
				}
			case 2:
				{
					_stprintf(szBuf, m_VecModuleInfo[iIndex].szModuleDesc);
					break;
				}
			}
		}
		...
		if(pControl->GetName() == _T("commandlist"))
		{
			...
			case 0:
				{
					_stprintf(szBuf, m_VecCommandInfo[iIndex].szModuleName);
					break;
				}
			case 1:
				{
					_stprintf(szBuf, m_VecCommandInfo[iIndex].szCommandID);
					break;
				}
			case 2:
				{
					_stprintf(szBuf, "%d", m_VecCommandInfo[iIndex].m_nCount);
					break;
				}
			case 3:
				{
					_stprintf(szBuf, "%d", m_VecCommandInfo[iIndex].m_nTimeCost);
					break;
				}
			}
		}
...
private:
	...
	CButtonUI* m_pMinBtn;
	CManagerConnect      m_ManagerConnect;
	CClientConnect       m_ClientConnect;
	CClientConnectUDP    m_ClientConnectUDP;
	vecModuleInfo        m_VecModuleInfo;
	vecCommandInfo       m_VecCommandInfo;
	vecWorkThreadInfo    m_VecWorkThreadInfo;
	vecClientConnectInfo m_VecClientConnectInfo;
	vecClientConnectInfo m_VecServerTCPConnectInfo;
	vecClientConnectInfo m_VecServerUDPConnectInfo;
	vecForbiddenIP       m_VecForbiddenIP;
	vecIPAccount         m_VecIPAccount;
};
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE /*hPrevInstance*/, LPSTR /*lpCmdLine*/, int nCmdShow)
{
	...
	pFrame->Create(NULL, _T("PurenessScopeServer客户端"), UI_WNDSTYLE_FRAME, 0L, 0, 0, 800, 600);
	pFrame->CenterWindow();
