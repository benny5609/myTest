...
bool CProactorClientInfo::Init(const char* pIP, int nPort, CProAsynchConnect* pProAsynchConnect, IClientMessage* pClientMessage)
{
	...
	if(-1 == nRet)
	{
		...
		TRACE("[CProactorClientInfo::Init]adrClient(%s:%d) error.\n", pIP, nPort);
		return false;
	}
...
bool CProactorClientInfo::Run()
{
	if(NULL != m_pProConnectClient)
	{
		...
		TRACE("[CProactorClientInfo::Run]Connect is exist.");
		return false;
	}
	...
	if(NULL == m_pProAsynchConnect)
	{
		...
		TRACE("[CProactorClientInfo::Run]m_pAsynchConnect is NULL.");
		return false;
	}
	...
	if(m_pProAsynchConnect->GetConnectState() == true)
	{
		...
		TRACE("[CProactorClientInfo::Run]m_pProAsynchConnect is run.");
		return false;
	}
	...
	if(m_pProAsynchConnect->connect(m_AddrServer) == -1)
	{
		...
		TRACE("[CProactorClientInfo::Run]m_pAsynchConnect open error(%d).", ACE_OS::last_error());
		return false;
	}
