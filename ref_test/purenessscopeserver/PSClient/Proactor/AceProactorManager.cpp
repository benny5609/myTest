...
void CAceProactor::Close()
{
	...
	{
		//SAFE_DELETE(m_pImpl);
		OUR_DEBUG((LM_ERROR, "[Delete]m_pProactor.\n"));
		...
		case Proactor_WIN32:    //这个功能限制于Windows操作的默认反应器，如果是COM服务器可以使用Proactor_WFMO_msg
			{
				...
				OUR_DEBUG((LM_ERROR, "[New]ACE_WIN32_Proactor().\n"));
				...
				//m_pImpl     = pWin32Proactor;
				OUR_DEBUG((LM_ERROR, "[New]ACE_Proactor().\n"));
...
int CAceProactor::open(void* args)
{
	...
	if(activate(THR_NEW_LWP | THR_BOUND | THR_JOINABLE, 1)  == -1)
	{
		m_blRun = false;
...
void CAceProactorManager::Close()
{
	...
	for(b; b!= e; b++)
	{
		...
		{
			OUR_DEBUG((LM_ERROR, "[Delete]pAceProactor.\n"));
...
{
	OUR_DEBUG((LM_ERROR, "[New]CAceProactor().\n"));
