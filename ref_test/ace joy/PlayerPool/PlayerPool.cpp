...
int LoadModuleData(CServerObject* pServerObject)
{
	...
	{
		pMessageManager->AddClientCommand(COMMAND_PLAYINSERT, &g_PlayerPoolCommand);
		pMessageManager->AddClientCommand(COMMAND_PLAYUPDATE, &g_PlayerPoolCommand);
		pMessageManager->AddClientCommand(COMMAND_PLAYDELETE, &g_PlayerPoolCommand);
		pMessageManager->AddClientCommand(COMMAND_PLAYSEACH,  &g_PlayerPoolCommand);
		pMessageManager->AddClientCommand(COMMAND_PLAYLOGIN,  &g_PlayerPoolCommand);
		pMessageManager->AddClientCommand(COMMAND_PLAYLOGOFF, &g_PlayerPoolCommand);
	}
	else
	{
...
int UnLoadModuleData()
{
	...
	if(g_pServerObject != NULL)
	{
		...
		if(NULL != pMessageManager)
		{
			pMessageManager->DelClientCommand(COMMAND_PLAYINSERT);
